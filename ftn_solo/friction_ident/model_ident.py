...
def calculate_params(torques, q, qv, qa, threshold):
    ...
    Q = np.vstack([qa, np.cos(q), np.sin(q), qv, np.arctan(qv/threshold)*2/np.pi]).T
    lam = 5 * np.eye(5,5)
    lam[4,4] = 10000
...
def new_torque_and_plot(t, torques, q, qv, qa, x, threshold, index):
    new_torques = evaluate(torques, q, qv, qa, x, threshold)
    fig[index], axs[index] = plt.subplots(2, 1)
    fig[index].suptitle("q"+str(index), fontsize=20)
    fig[index].subplots_adjust(
        left=0.05, right=0.993, top=0.945, bottom=0.045, wspace=0.06, hspace=0.114)
    axs[index][0].plot(t[20:-20], np.vstack(
    ...
    axs[index][1].plot(t[20:-20], (torques - new_torques)[20:-20],
    ...
    axs[index][1].set_ylabel("tau [Nm]", fontsize=20 - 5)
    axs[index][1].tick_params(axis='both', which='major', labelsize=20 - 7)
    axs[index][1].set_title("Torque error {}".format(average_sum_of_squares(torques - new_torques)))
...
def main(args=None):
    ...
    for i in range(3):
        ...
        while index < joints_num:
            ...
            if plot_on:
                new_torque_and_plot(
                    t, torques[:, index], q[:, index], qv_filtered, qa_filtered, x[:, index], thresholds[index], index)
