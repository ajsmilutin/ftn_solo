...
class ControllerIdent():
    states = ['start', 'move_knee', 'position_calf', 'move_hip',
              'position_thigh', 'rotate_hip', 'return_to_start', 'idle']
    chirp_duration = 5.0
    chirp_F0 = 0.4
    chirp_F1 = 0.8
    ...
    def __init__(self, num_of_joints) -> None:
        self.machine = Machine(
            model=self, states=ControllerIdent.states, initial='start')
        self.machine.add_transition(
            'tick', 'start', 'move_knee', conditions='prepare_start')
        self.machine.add_transition(
            'tick', 'move_knee', 'position_calf', conditions='move_joint')
        self.machine.add_transition(
            'tick', 'position_calf', 'move_hip', conditions='prepare_calf')
        self.machine.add_transition(
            'tick', 'move_hip', 'position_thigh', conditions='move_joint')
        self.machine.add_transition(
            'tick', 'position_thigh', 'rotate_hip', conditions='prepare_thigh')
        self.machine.add_transition(
            'tick', 'rotate_hip', 'return_to_start', conditions='move_joint')
        self.machine.add_transition(
            'tick', 'return_to_start', 'idle', conditions='go_to_start')
        self.machine.add_transition(
        ...
        self.chirp_t = np.arange(
            0, self.chirp_duration + self.dT, self.dT / 2.0, dtype=np.float64)
        phase = 270
        self.chirp = chirp(self.chirp_t, self.chirp_F0,
                           self.transition_end, self.chirp_F1, phi=phase)
    ...
    def move_joint(self, t, q, qv):
        ...
        try:
            chirp_index = np.where(self.chirp_t >= t -
                                   self.transition_start)[0][0]
