...
class TaskMoveBase(TaskWithInitPose):
        ...
        if self.phase >= 0:
            for motion in self.sequence[self.phase].motions:
                if type(motion) is EEFMotionData:
                    self.tmp_friction_cones[motion.eef_index] = (
                        self.estimator.create_friction_cone(
                            motion.eef_index,
                            motion.rotation,
    ...
    def fix_eef(self):
        ...
        if self.phase > 0:
            ...
            for motion in self.sequence[phase].motions:
                if type(motion) is EEFMotionData:
                    self.estimator.set_fixed(motion.eef_index, motion.rotation)
    ...
    def update_eef_trajectory_start(self, t, q, qv, sensors):
        ...
        self.trajectory_planner.update_eef_trajectory(
            t,
    ...
    def init_qp(self):
        self.controller = WholeBodyController(
            ...
            self.friction_cones,
            self.max_torque,
            yaml.dump(self.config["whole_body"]),
        )
    ...
    def moving_eef(self, t, q, qv, sensors):
        ...
        if not self.finished:
            ...
            if (
                ...
                self.status_publisher.publish(
                    String(
                        data="EEF starting comutation done, phase: {}".format(
                            self.phase
                        )
                    )
                )
